...
export default function NewProduct() {
  ...
  const L = {
    ...
    saveLabel: isLawyerLayout ? 'Cadastrar Serviço' : (isPartMode ? 'Cadastrar Peça' : 'Cadastrar Produto'),
    pageTitle: isLawyerLayout ? 'Novo Serviço' : (isPartMode ? 'Nova Peça' : 'Novo Produto'),
  }
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!store || !newCategory.name) return
    
    try {
      const { data, error } = await supabase
        .from('categories')
        .insert([{ name: newCategory.name, image_url: newCategory.image_url, store_id: store.id }])
        .select()
      ...
      const { error } = await supabase.from('products').insert([
        {
          ...
          is_service: isServiceType,
          images: allImages,
          has_variations: hasVariations,
          variation_options: hasVariations ? variationOptions.map(o => ({ name: o.name.trim(), values: o.valuesString.split(',').map(v => v.trim()).filter(Boolean) })) : [],
          variation_skus: hasVariations ? variationSkus : []
        }
      ])

      if (error) throw error

      toast.success(`${L.singular} cadastrado${isPartMode ? 'a' : ''} com sucesso!`)
    ...
    <div style={{ width: '100%', maxWidth: '1400px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem', width: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: '2rem', width: '100%', alignItems: 'flex-start' }}>
          ...
          {/* Coluna Esquerda */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', width: 'calc(40% - 1rem)', flexShrink: 0 }}>
            ...
            <div className="glass-card" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder={isServicesOnly ? "Ex: Divórcio Consensual" : "Ex: Tênis Nike Revolution 6"}
                />
              </div>
                ...
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    <button
                      ...
                      style={{ padding: '0.75rem', background: 'rgba(99, 102, 241, 0.1)', color: 'var(--primary)', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, whiteSpace: 'nowrap' }}
                    >
                      + Nova
                    </button>
                  </div>
          ...
          {/* Coluna Direita */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', width: 'calc(60% - 1rem)', flexShrink: 0 }}>
          ...
          <div className="glass-card" style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%' }}>
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              {variationSkus.map((skuObj, skuIdx) => (
                <div key={skuIdx} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.2fr 1fr 2fr auto', gap: '1.5rem', alignItems: 'flex-end', background: 'var(--background)', padding: '1.5rem', border: '1px solid var(--border)', borderRadius: '12px' }}>
        ...
        <div className="glass-card" style={{ padding: '2rem', display: 'flex', justifyContent: 'flex-end', gap: '1.5rem', width: '100%' }}>
          ...
          <button type="submit" disabled={loading} className="btn-primary" style={{ width: '250px' }}>
            {loading ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}
            {loading ? 'Cadastrando...' : L.saveLabel}
          </button>
        </div>
      ...
      {showCategoryModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(15, 23, 42, 0.4)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '2rem' }}>
          <div className="glass-card" style={{ maxWidth: '400px', width: '100%', padding: '2rem', position: 'relative', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.15)' }}>
            <button
            ...
            <h3 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '1.5rem', color: 'var(--foreground)' }}>Nova Categoria</h3>

            <form onSubmit={handleCreateCategory} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div className="form-group">
                <label>Nome da {isServicesOnly ? 'Área de Atuação' : 'Categoria'}</label>
                <input
                  type="text"
                  required
                  value={newCategory.name}
                  onChange={(e) => setNewCategory({...newCategory, name: e.target.value})}
                  placeholder={isServicesOnly ? "Ex: Direito Civil, Direito Trabalhista" : "Ex: Eletrônicos, Roupas"}
                />
              </div>
              ...
              <button
                ...
                style={{ padding: '0.875rem', width: '100%', fontSize: '1rem' }}
              >
                Criar Categoria
              </button>
            </form>
