...
export default async function StoreFront({ params, searchParams }: { params: Promise<{ domain: string }>, searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  return (
    <div style={{ 
      ...
      {/* 8. BOTÃO DO WHATSAPP FLUTUANTE DO LOJISTA */}
      {storeWhatsapp && (
        <a 
          href={`https://wa.me/55${storeWhatsapp.replace(/\D/g, '')}?text=${encodeURIComponent('Olá! Gostaria de saber mais sobre os produtos da loja.')}`}
          target="_blank" 
